 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */var xc=Le,Ec=Symbol.for("react.element"),Tc=Symbol.for("react.fragment"),Pc=Object.prototype.hasOwnProperty,Nc=xc.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner,jc={key:!0,ref:!0,__self:!0,__source:!0};function nu(e,n,t){var r,l={},o=null,i=null;t!==void 0&&(o=""+t),n.key!==void 0&&(o=""+n.key),n.ref!==void 0&&(i=n.ref);for(r in n)Pc.call(n,r)&&!jc.hasOwnProperty(r)&&(l[r]=n[r]);if(e&&e.defaultProps)for(r in n=e.defaultProps,n)l[r]===void 0&&(l[r]=n[r]);return{$$typeof:Ec,type:e,key:o,ref:i,props:l,_owner:Nc.current}}tl.Fragment=Tc;tl.jsx=nu;tl.jsxs=nu;Qs.exports=tl;var y=Qs.exports,Gl={},tu={exports:{}},ye={},ru={exports:{}},lu={};/**
 * @license React
 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */(function(e){function n(E,j){var _=E.length;E.push(j);e:for(;0<_;){var $=_-1>>>1,X=E[$];if(0<l(X,j))E[$]=j,E[_]=X,_=$;else break e}}function t(E){return E.length===0?null:E[0]}function r(E){if(E.length===0)return null;var j=E[0],_=E.pop();if(_!==j){E[0]=_;e:for(var $=0,X=E.length,er=X>>>1;$<er;){var gn=2*($+1)-1,wl=E[gn],yn=gn+1,nr=E[yn];if(0>l(wl,_))yn<X&&0>l(nr,wl)?(E[$]=nr,E[yn]=_,$=yn):(E[$]=wl,E[gn]=_,$=gn);else if(yn<X&&0>l(nr,_))E[$]=nr,E[yn]=_,$=yn;else break e}}return j}function l(E,j){var _=E.sortIndex-j.sortIndex;return _!==0?_:E.id-j.id}if(typeof performance=="object"&&typeof performance.now=="function"){var o=performance;e.unstable_now=function(){return o.now()}}else{var i=Date,s=i.now();e.unstable_now=function(){return i.now()-s}}var u=[],c=[],p=1,h=null,m=3,S=!1,w=!1,k=!1,F=typeof setTimeout=="function"?setTimeout:null,f=typeof clearTimeout=="function"?clearTimeout:null,a=typeof setImmediate<"u"?setImmediate:null;typeof navigator<"u"&&navigator.scheduling!==void 0&&navigator.scheduling.isInputPending!==void 0&&navigator.scheduling.isInputPending.bind(navigator.scheduling);function d(E){for(var j=t(c);j!==null;){if(j.callback===null)r(c);else if(j.startTime<=E)r(c),j.sortIndex=j.expirationTime,n(u,j);else break;j=t(c)}}function v(E){if(k=!1,d(E),!w)if(t(u)!==null)w=!0,yl(x);else{var j=t(c);j!==null&&Sl(v,j.startTime-E)}}function x(E,j){w=!1,k&&(k=!1,f(N),N=-1),S=!0;var _=m;try{for(d(j),h=t(u);h!==null&&(!(h.expirationTime>j)||E&&!Pe());){var $=h.callback;if(typeof $=="function"){h.callback=null,m=h.priorityLevel;var X=$(h.expirationTime<=j);j=e.unstable_now(),typeof X=="function"?h.callback=X:h===t(u)&&r(u),d(j)}else r(u);h=t(u)}if(h!==null)var er=!0;else{var gn=t(c);gn!==null&&Sl(v,gn.startTime-j),er=!1}return er}finally{h=null,m=_,S=!1}}var T=!1,P=null,N=-1,W=5,L=-1;function Pe(){return!(e.unstable_now()-L<W)}function at(){if(P!==null){var E=e.unstable_now();L=E;var j=!0;try{j=P(!0,E)}finally{j?ct():(T=!1,P=null)}}else T=!1}var ct;if(typeof a=="function")ct=function(){a(at)};else if(typeof MessageChannel<"u"){var Ii=new MessageChannel,oc=Ii.port2;Ii.port1.onmessage=at,ct=function(){oc.postMessage(null)}}else ct=function(){F(at,0)};function yl(E){P=E,T||(T=!0,ct())}function Sl(E,j){N=F(function(){E(e.unstable_now())},j)}e.unstable_IdlePriority=5,e.unstable_ImmediatePriority=1,e.unstable_LowPriority=4,e.unstable_NormalPriority=3,e.unstable_Profiling=null,e.unstable_UserBlockingPriority=2,e.unstable_cancelCallback=function(E){E.callback=null},e.unstable_continueExecution=function(){w||S||(w=!0,yl(x))},e.unstable_forceFrameRate=function(E){0>E||125<E?console.error("forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported"):W=0<E?Math.floor(1e3/E):5},e.unstable_getCurrentPriorityLevel=function(){return m},e.unstable_getFirstCallbackNode=function(){return t(u)},e.unstable_next=function(E){switch(m){case 1:case 2:case 3:var j=3;break;default:j=m}var _=m;m=j;try{return E()}finally{m=_}},e.unstable_pauseExecution=function(){},e.unstable_requestPaint=function(){},e.unstable_runWithPriority=function(E,j){switch(E){case 1:case 2:case 3:case 4:case 5:break;default:E=3}var _=m;m=E;try{return j()}finally{m=_}},e.unstable_scheduleCallback=function(E,j,_){var $=e.unstable_now();switch(typeof _=="object"&&_!==null?(_=_.delay,_=typeof _=="number"&&0<_?$+_:$):_=$,E){case 1:var X=-1;break;case 2:X=250;break;case 5:X=1073741823;break;case 4:X=1e4;break;default:X=5e3}return X=_+X,E={id:p++,callback:j,priorityLevel:E,startTime:_,expirationTime:X,sortIndex:-1},_>$?(E.sortIndex=_,n(c,E),t(u)===null&&E===t(c)&&(k?(f(N),N=-1):k=!0,Sl(v,_-$))):(E.sortIndex=X,n(u,E),w||S||(w=!0,yl(x))),E},e.unstable_shouldYield=Pe,e.unstable_wrapCallback=function(E){var j=m;return function(){var _=m;m=j;try{return E.apply(this,arguments)}finally{m=_}}}})(lu);ru.exports=lu;var _c=ru.exports;/**
 * @license React
...
**User.prototype** по умолчанию = \`{ constructor: User }\``,score:3.5},{name:"Подмена существующего контекста",level:"2",content:`**call(ctx, a, b)** — вызывает сразу, аргументы через запятую
...
**Практика:** Vue 3 (реактивность), MobX, Immer (Redux Toolkit).`,score:3}]},{id:"browser",title:"БРАУЗЕР И ЕГО ИНСТРУМЕНТЫ",icon:"🌐",color:"#4FC3F7",topics:[{name:"devTools (network, application)",level:"2",content:`**Network:** мониторинг HTTP-запросов, время загрузки, размеры, заголовки, тело ответа. Фильтрация по типу (XHR, JS, CSS, Img). Throttling (3G, offline).
...
**Console:** логирование, выполнение JS, фильтрация по уровням.`},{name:"breakpoints",level:"2",content:`**Типы breakpoints в DevTools:**
...
**Классы:** classList.add, remove, toggle, contains.`},{name:"iframe",level:"3",content:`**iframe** — встраивает другую HTML-страницу внутрь текущей.
...
**Минусы:** нет window/document, увеличенный TTFB, нагрузка на сервер.`,score:3.5},{name:"Form managers",level:"3",content:`**React Hook Form** — неконтролируемые компоненты + useRef. Минимум ререндеров.
**Formik** — контролируемые компоненты. Каждое нажатие → ререндер.

React Hook Form + **Zod/Yup** — валидация по схеме с выводом типов.`,score:3},{name:"React паттерны. Compound components. render-props",level:"4",content:`**Compound Components:** набор связанных компонентов через Context.
\`\`\`jsx
<Table>
...
**Плюс:** тяжёлые библиотеки не попадают в бандл клиента. Прямой доступ к БД, файловой системе.`,score:4}]}],zd=[{id:"network",title:"СЕТЬ",icon:"🔗",color:"#66BB6A",topics:[{name:"ip-адрес и hostName",level:"1",content:`**IP-адрес** — уникальный числовой идентификатор устройства в сети.
...
**HTTPS** — HTTP + TLS шифрование.`},{name:"Понимание протокола HTTP/2",level:"3",content:`**Улучшения над HTTP/1.1:**
...
\`\`\`
В JS модули — уже singleton (экспорт кэшируется).`},{name:"Навык использования и определения паттерна в коде: Decorator",level:"3",content:`**Decorator** — оборачивает объект/функцию, добавляя поведение без изменения оригинала.
...
**В JS используются оба:** React (ФП — компоненты как чистые функции, иммутабельный state) + классы, наследование.`},{name:"SOLID, Cohesion и Coupling",level:"3",content:`**S** — Single Responsibility: класс/модуль — одна ответственность
...
\`\`\``,score:4},{name:"Семантичные теги, базовые теги",level:"1",content:`Семантические теги: header, main, footer, nav, section, article, aside, figure.
...
SVG — лёгкий и чёткий на любом экране. Для сложных изображений может быть тяжелее растра.`,score:2},{name:"browserslist",level:"4",content:'Конфиг, определяющий целевые браузеры. Читают: Autoprefixer, Babel, PostCSS Preset Env, ESLint.\n\n```json\n"browserslist": ["> 1%", "last 2 versions", "not dead"]\n```\n\nВсе инструменты работают согласованно по одному конфигу.',score:2.5},{name:"Доступность (aria, tabindex & etc.)",level:"4",content:'**tabindex:** 0 — в порядке DOM, -1 — только программно (focus()), положительный — антипаттерн.\n\n**ARIA:** aria-label, aria-hidden, aria-expanded, aria-live, role.\n\n**Первое правило ARIA:** если есть нативный HTML-элемент — используй его вместо ARIA. `<button>` лучше `<div role="button">`.',score:3}]},{id:"async",title:"АСИНХРОННОСТЬ",icon:"🔄",color:"#FF6B6B",topics:[{name:"XMLHttpRequest, fetch",level:"2",content:`**XMLHttpRequest** — колбэки. **fetch** — промисы.
...
В первом: err НЕ ловит ошибки из ok. Во втором: catch ловит всё.`},{name:"Event loop",level:"2",content:"Подробный разбор в теме «Механизм Event loop». Отдельная оценка: понимание Web API, callstack, очередей."}]},{id:"ts",title:"TS",icon:"🔷",color:"#3178C6",topics:[{name:"Основные типы данных в TS",level:"2",content:`**Примитивы:** string, number, boolean, null, undefined, bigint, symbol.
...
**Способы:** JWT, Session cookies, OAuth 2.0, SSO.`},{name:"npm audit. SCA и SAST",level:"2",content:`**npm audit** — проверка зависимостей на известные уязвимости.
...
Кэширование на edge-серверах. Защита от DDoS.`},{name:"Lazy-loading, оптимизация критичного пути",level:"4",content:`**Critical Rendering Path:** HTML → DOM + CSSOM → Render Tree → Layout → Paint.
